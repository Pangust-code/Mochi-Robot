...
| 05 | `05_prueba_wifi` | WiFi integrado | Redes 2.4 GHz, WPA2, HTTP como puerta a internet |
| 07 | `07_prueba_conexion_microfono` | Micrófono INMP441 | Protocolo I2S, sincronía de audio digital |
| 09 | `09_prueba_sonido_microfono` | Micrófono INMP441 | Amplitud, muestreo, umbrales de detección |

> Las pruebas 04, 06, 08 son opcionales en el taller. El orden recomendado es 01 → 02 → 03 → 05 → 07 → 09.
...
- ESP32-C6 Supermini conectado al PC por USB-C
...
**¿Qué aprenderás?** El ESP32 soporta WPA2-Personal (red de casa) y WPA2-Enterprise (red universitaria con usuario y contraseña separados). Una vez conectado, puede hacer llamadas HTTP a cualquier API en internet — aquí solo verificamos la conexión.

Abre `07_prueba_wifi/prueba_wifi.ino` y edita el arreglo `networks[]`:
...
```powershell
arduino-cli compile --fqbn esp32:esp32:esp32c6 03_firmware/esp32c6/pruebas/07_prueba_wifi
arduino-cli upload -p COM3 --fqbn esp32:esp32:esp32c6 03_firmware/esp32c6/pruebas/07_prueba_wifi
...
**Para pensar:** Una vez que el ESP32 tiene IP, cualquier dispositivo en la misma red puede comunicarse con él. ¿Qué implica eso para el servidor de transcripción de la Prueba 10?
...
3. Prueba tocar con un lápiz o con una moneda. ¿El sensor reacciona igual?
...
```powershell
arduino-cli compile --fqbn esp32:esp32:esp32c6 03_firmware/esp32c6/pruebas/07_prueba_conexion_microfono
arduino-cli upload -p COM3 --fqbn esp32:esp32:esp32c6 03_firmware/esp32c6/pruebas/07_prueba_conexion_microfono
arduino-cli monitor -p COM3 -b esp32:esp32:esp32c6 -c baudrate=115200
```
...
Guía completa → [09_prueba_sonido_microfono/README.md](09_prueba_sonido_microfono/README.md)
...
**Reto disponible:** El [Reto 1 — Nueva melodía](../retos/reto-1-melodia/) tiene un sketch con el array de notas vacío. Llénalo con cualquier canción. Solo necesitas frecuencias (Hz) y duraciones (ms).
...
**¿Qué aprenderás?** El ESP32 puede consultar cualquier API REST. OpenWeatherMap devuelve temperatura, humedad y pronóstico en JSON. El sketch lo parsea y lo muestra en la OLED con 3 vistas distintas.

Antes de compilar, edita `08_prueba_clima/prueba_clima.ino`:
...
```powershell
arduino-cli compile --fqbn esp32:esp32:esp32c6 03_firmware/esp32c6/pruebas/08_prueba_clima
arduino-cli upload -p COM3 --fqbn esp32:esp32:esp32c6 03_firmware/esp32c6/pruebas/08_prueba_clima
```

| Acción | Resultado |
|--------|-----------|
| Tap corto | Cicla entre vistas (actual → pronóstico → 3 días) |
| Hold largo | Fuerza actualización inmediata |
| Automático | Actualiza cada 10 minutos |

**Para explorar:** El JSON de OpenWeatherMap tiene muchos más campos de los que se muestran: `feels_like`, `pressure`, `visibility`. ¿Puedes agregar uno a alguna de las vistas? Solo necesitas encontrar su nombre en el JSON del monitor serie y parsearlo igual que temperatura.
...
## Prueba 10 — Transcripción de voz

**¿Qué aprenderás?** El ESP32 graba audio en RAM, lo empaqueta como WAV y lo envía por HTTP POST a un servidor Python local. Ese servidor usa Whisper (modelo de reconocimiento de voz de OpenAI) para transcribir el audio y devuelve el texto.
...
Guía completa → [10_transcripcion/README.md](10_transcripcion/README.md)

**Para explorar:** El texto transcrito llega al ESP32 como un string. ¿Qué podrías hacer con él? La siguiente prueba ([11 — Código funcional](11_codigo_funcional_transcripcion/)) lo envía a un servidor de IA que detecta la intención y cambia el mood del robot.
...
| Pantalla en blanco tras subir | Pines I2C invertidos | SDA = GPIO 6, SCL = GPIO 7 |
...
**[03_firmware/esp32c6/retos/](../retos/) — Fase 3: Easter Egg — Juegos en OLED**
