...
> **Prerrequisito:** [software/](../software/) — haber leído las funciones del robot.
...
└── reto-10-nueva-api/        ← Reto nuevo: conecta tu propia API REST
```
...
```powershell
arduino-cli compile --fqbn esp32:esp32:esp32c3 03_firmware/esp32c3/retos/reto-5-chromedino
arduino-cli upload -p COM3 --fqbn esp32:esp32:esp32c3 03_firmware/esp32c3/retos/reto-5-chromedino
```
...
```powershell
arduino-cli compile --fqbn esp32:esp32:esp32c3 03_firmware/esp32c3/retos/reto-6-flappybird
arduino-cli upload -p COM3 --fqbn esp32:esp32:esp32c3 03_firmware/esp32c3/retos/reto-6-flappybird
```
...
**Qué modificar:** Función `tetrisTheme()` en `software.ino`.
...
**Qué modificar:** Tres lugares en `software.ino`: la constante `MOOD_COUNT`, la función `drawEyelidMask()` y el `switch` dentro de `updateEyes()`.
...
3. Copia el `.bin` a `esp32c3/software/data/`.
...
```powershell
arduino-cli compile --fqbn esp32:esp32:esp32c3 03_firmware/esp32c3/retos/reto-4-aplausos
arduino-cli upload -p COM3 --fqbn esp32:esp32:esp32c3 03_firmware/esp32c3/retos/reto-4-aplausos
arduino-cli monitor -p COM3 -b esp32:esp32:esp32c3 -c baudrate=115200
```

**Fase 2 — Integrar como Modo 5:** cuando la detección funcione, el README te guía con los 3 cambios exactos en `software.ino`.
...
**Qué necesitas:** WiFi funcionando (Prueba 08) y, opcionalmente, haber completado la prueba de clima (que ya hace exactamente este patrón).
...
**← Anterior:** [software/](../software/) — funciones del robot
