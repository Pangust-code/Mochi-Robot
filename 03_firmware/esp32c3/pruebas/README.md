...
| 05 | `05_prueba_conexion_microfono` | Micrófono INMP441 | I2S | Pines detectados, sin error |
| 06 | `06_diagnostico_hardware` | Todos los componentes | I2C/I2S/GPIO/PWM | Diagnóstico completo en pantalla |
| 08 | `08_prueba_wifi` | WiFi integrado | 802.11 | IP asignada en Monitor Serie |
| 09 | `09_prueba_clima` | WiFi + API OpenWeatherMap | HTTP/JSON | Temperatura y humedad en pantalla |
| 10 | `10_prueba_sonido_microfono` | Micrófono INMP441 | I2S | Barra de volumen reactiva |

> Las pruebas 04, 07, 11, 12 y 13 son opcionales o avanzadas. El orden recomendado es 01 → 02 → 03 → 05 → 06 → 08 → 09 → 10.
...
> **Nota:** Usa siempre 3.3V. El ESP32-C3 Super Mini no tolera 5V en sus pines GPIO.
...
Antes de compilar, abre el archivo `08_prueba_wifi/prueba_wifi.ino` y edita el arreglo `networks[]` con tus credenciales:
...
| `Connection failed` |   Red de 5 GHz — el ESP32-C3 Super Mini solo soporta 2.4 GHz |
...
| Valor siempre en cero |   Verifica WS→GPIO1, SCK→GPIO4, SD→GPIO3 |
...
> Guarda estos valores. En la Fase 5 los ingresarás en la sección de configuración de `software.ino`.

---

## Prueba 09 — Estación meteorológica (clima)

**¿Qué verifica?** Que el ESP32-C3 Super Mini puede consultar la API de OpenWeatherMap por WiFi y mostrar temperatura, humedad y viento en la pantalla OLED.

Antes de compilar, abre `09_prueba_clima/prueba_clima.ino` y edita el arreglo `networks[]` y la clave de API:
...
| Acción | Resultado |
|--------|-----------|
| Toque corto | Cicla entre vistas (condición actual → pronóstico del día) |
...
| Pantalla en blanco tras subir sketch | Pines I2C invertidos | SDA = GPIO 8, SCL = GPIO 9 (no al revés) |
