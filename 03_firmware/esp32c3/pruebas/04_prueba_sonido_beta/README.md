# 🤖 Suite de Pruebas de Hardware y Diagnóstico: ESP32-C3 Super Mini
...
Para la ejecución correcta de cualquiera de los cuatro scripts de diagnóstico, la placa de desarrollo **ESP32-C3 Super Mini** debe conectarse siguiendo este mapa estricto de pines e interconexiones lógicas:

| Componente Periférico | Pin del Componente | Pin ESP32-C3 Super Mini | Función Lógica | Descripción del Canal de Control |
| :--- | :--- | :--- | :--- | :--- |
...
| **OLED SH1106 128×64**| SDA | **GPIO 8** | Bus Datos I2C | Línea bidireccional de datos gráficos. |
| **OLED SH1106 128×64**| SCL | **GPIO 9** | Bus Reloj I2C | Señal de sincronización de reloj a 400 kHz. |
| **TTP223 Touch Sensor**| SIG / OUT | **GPIO 2** | Entrada Digital | Captura de flancos lógicos de interacción. |
| **Buzzer Pasivo / Spk** | SIG / PWM | **GPIO 10** | Salida PWM | Generación de frecuencias de audio variables. |

> ⚠️ **Nota de Seguridad:** Todos los periféricos operan nativamente a un nivel lógico de **3.3V**. No alimente el sensor táctil ni la pantalla con la salida de 5V para evitar daños permanentes en las compuertas lógicas del ESP32-C3 Super Mini.
...
Entorno de Desarrollo: Abra su entorno de configuración (Arduino IDE o PlatformIO) y configure la placa de desarrollo seleccionando ESP32-C3 Super Mini o su equivalente directo.
